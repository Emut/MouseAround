...
__declspec(dllexport) LRESULT CALLBACK KeyboardEvent(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	KBDLLHOOKSTRUCT* pKbStruct = (KBDLLHOOKSTRUCT*)lParam;
	printf("%c", pKbStruct->vkCode);
	if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN)
		printf("Dwn\n");
	else
		printf("Up\n");
	if (pKbStruct->vkCode == 'A')
		return -1;
...
int main(int argc, char** argv) {
	...
	DWORD dwMouseThread;
	hMouseThread = CreateThread(NULL, NULL,
		(LPTHREAD_START_ROUTINE)MouseLogger, (LPVOID)argv[0], NULL, &dwMouseThread);

	HANDLE hKbThread;
	DWORD dwKbThread;
	hKbThread = CreateThread(NULL, NULL,
		(LPTHREAD_START_ROUTINE)KeyboardLogger, (LPVOID)argv[0], NULL, &dwKbThread);
	Sleep(60 * 1000);
	return 0;
}*/